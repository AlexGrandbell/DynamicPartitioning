...
bool RAM::AllocateMemory(int processId) {
   ...
   queue<int> tempFreePartitionSpaces;
   //清空
    while(!tempFreePartitionSpaces.empty()) {
        tempFreePartitionSpaces.pop();
    }
    while(!partitionFreeSpaces.empty()) {
        int partitionId = partitionFreeSpaces.top();
        partitionFreeSpaces.pop();
        tempFreePartitionSpaces.push(partitionId);
        if(partitionSpacesMap[partitionId].assignedSize >= processesMap[processId].needSize) {
            ...
            if (partitionSpacesMap[partitionId].assignedSize > processesMap[processId].needSize) {
                ...
                while (it2 != partitionAllSpaces.end() &&
                       partitionSpacesMap[*it2].startAddress < newPartitionSpace.startAddress) {
                    it2++;
                }
                partitionAllSpaces.insert(it2, newPartitionSpace.partitionId);
                //直接插入空闲分区优先队列到合适位置
                partitionFreeSpaces.push(newPartitionSpace.partitionId);
                //输出分割结果
            ...
            //将临时优先队列放回空闲分区优先队列
            while(!tempFreePartitionSpaces.empty()) {
                partitionFreeSpaces.push(tempFreePartitionSpaces.front());
                tempFreePartitionSpaces.pop();
            }
            return true;
        }
    ...
    for(auto it = partitionAllSpaces.begin(); it != partitionAllSpaces.end(); it++) {
        ...
        if(partitionSpacesMap[*it].isFree) {
            ...
            if(nextIt != partitionAllSpaces.end() && partitionSpacesMap[*nextIt].isFree) {
                ...
                queue<int> tempFreePartitionSpaces;
                while(!partitionFreeSpaces.empty()) {
                    if(partitionFreeSpaces.top() == *nextIt) {
                        partitionFreeSpaces.pop();
                        break;
                    }else{
                        tempFreePartitionSpaces.push(partitionFreeSpaces.top());
                        partitionFreeSpaces.pop();
                    }
                }
                while(!tempFreePartitionSpaces.empty()) {
